...
class ProjectsPage(TopNavigateBar):
    ...
    def delete_workspace(self) -> None:
        ...
        name = self.driver.find_element(*self._DELETE_WORKSPACE_FIELD).get_attribute(
            "placeholder"
        )
    ...
    def rename_workspace(self, old_name: str, new_name: str) -> None:
        ...
        flag = any(
            old_name in workspaces_text_list[i]
    ...
    def create_new_project(self) -> None:
        ...
        elif self.is_elem_displayed(
            self.driver.find_element(*self._CREATE_PROJECT_BUTTON)
        ):
            self.click(self._CREATE_NEW_WORKSPACE_BUTTON)
    ...
    def delete_project(self, project_name: str, status="confirm") -> None:
        ...
        if status == StatusEnum.CANCEL.value:
            ...
            self.wait.until(
                expected_conditions.invisibility_of_element(deleted_project)
            )
    ...
    def get_workspaces_number(self) -> int:
        self.wait.until(
            expected_conditions.invisibility_of_element_located(
                self._NEW_WORKSPACE_NAME_FIELD
            )
        )
        workspaces = self.wait.until(
            expected_conditions.visibility_of_all_elements_located(self._WORKSPACE_LIST)
    ...
    @allure.step(
        "Get number of projects displayed next to main workspace (My Workspace) name"
    )
    def get_projects_number_from_workspace(self) -> int:
        workspaces = self.wait.until(
            expected_conditions.visibility_of_all_elements_located(self._WORKSPACE_LIST)
        )
        number = workspaces[0].find_element(
            *self._NUMBER_OF_PROJECTS_IN_WORKSPACE_BLOCK
        )
    ...
    def is_workspace_found(self, workspace_name: str) -> bool:
        self.wait.until(
            expected_conditions.invisibility_of_element_located(self._RENAME_FIELD)
        )
        workspaces = self.wait.until(
            expected_conditions.visibility_of_all_elements_located(self._WORKSPACE_LIST)
        )
    ...
    def is_project_found(self, project_name: str) -> bool:
        ...
        return all(
            project_name == project_title.text.lower()
